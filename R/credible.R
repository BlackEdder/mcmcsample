...
inside.ci <- function( samples, ci = 0.9, method = "bin", ... )
{
  ...
  smpls <- as.data.frame(samples)
  discard <- floor(nrow(smpls)-nrow(smpls)*ci)
  inside <- rep(T,nrow(smpls))
  if (method == "chull")
    ids <- ci.chull( smpls, discard, ... )
  else if (method == "minmax")
    ids <- ci.minmax( smpls, discard, ... )
  else if (method == "bin")
    ids <- ci.bin( smpls, discard, ... )
